...
def make_unc(model, n_samples, all_conds):
    ...
    for k in all_conds:
        if k == "c_crossattn":
            ...
            uc[k] = (
                torch.ones_like(all_conds[k]) * model.low_scale_model.max_noise_level
            )
...
config = hf_hub_download(
    repo_id="lambdalabs/stable-diffusion-super-res", filename="sd-superres-config.yaml"
)
ckpt = hf_hub_download(
    repo_id="lambdalabs/stable-diffusion-super-res", filename="sd-superres-pruned.ckpt"
)
...
inputs = [
    ...
    gr.Dropdown(
        choices=[512, 1024],
        label="target_resolution (output is resized to square)",
        value=1024,
    ),
    gr.Dropdown(
        choices=[128, 256, 512, 1024], label="Downsize to this first", value=512
    ),
